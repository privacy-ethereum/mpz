#!/bin/bash
# Build the sample guest (vm-cost-explorer-sample) to wasm and embed it as base64
...
GUEST_DIR="$ROOT/crates/vm-cost-explorer-sample"
...
WASM="$GUEST_DIR/target/wasm32-unknown-unknown/wasm/vm_cost_explorer_sample.wasm"
...
out = (
    "// Embedded sample guest module (crates/vm-cost-explorer-sample): exports\n"
    "//   sha256(ptr, len, out) and json_parse(ptr, len).\n"
    "// Loaded by default so the page works out of the box.\n"
    "// Regenerate with ./crates/vm-cost-explorer/embed-sample.sh\n"
    'export const sampleName = "vm_cost_explorer_sample.wasm";\n'
