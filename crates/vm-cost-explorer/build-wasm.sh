#!/bin/bash
# Build the in-browser profiler: compile vm-cost-explorer to wasm32-unknown-unknown
...
CRATE_VER="$(cargo tree -p vm-cost-explorer -i wasm-bindgen --target wasm32-unknown-unknown 2>/dev/null \
...
echo "Building vm-cost-explorer for wasm32-unknown-unknown..."
cargo build -p vm-cost-explorer --target wasm32-unknown-unknown --release

echo "Generating JS bindings (--target web)..."
wasm-bindgen --target web --no-typescript \
    --out-dir "$SCRIPT_DIR/pkg" \
    "$ROOT/target/wasm32-unknown-unknown/release/vm_cost_explorer.wasm"
