...
# Build from within the crate directory so cargo discovers its
# `.cargo/config.toml` (memory limits and the `__heap_base` export). Cargo
# reads config from the working directory upward, not from `--manifest-path`,
# so the root config alone would otherwise win.
cd "$PROGRAMS_DIR"
cargo +nightly build \
