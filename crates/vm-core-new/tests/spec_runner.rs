...
use mpz_vm_core_new::{Param, Vm, VmError, ideal::Instance, value::Value};
use wast::{
...
/// Uses local mode (single-party, no I/O) for testing.
struct VmPair {
    vm_a: Instance,
    vm_b: Instance,
    module: Module,
...
impl VmPair {
    fn new(module: Module) -> Result<Self, String> {
        let vm_a = Instance::new(module.clone()).map_err(|e| format!("{:?}", e))?;
        let vm_b = Instance::new(module.clone()).map_err(|e| format!("{:?}", e))?;
        Ok(Self { vm_a, vm_b, module })
    }

    /// Reset VMs to initial state (after a trap corrupts them).
    fn reset(&mut self) -> Result<(), String> {
        self.vm_a = Instance::new(self.module.clone()).map_err(|e| format!("{:?}", e))?;
        self.vm_b = Instance::new(self.module.clone()).map_err(|e| format!("{:?}", e))?;
        Ok(())
    }

    /// Execute a function on both VMs and return results.
    fn execute(
        &mut self,
        func_idx: u32,
        args_a: Vec<Param>,
        ...
        let (result_a, result_b) = block_on(try_join(
            self.vm_a.call(&mut ctx_a, func_idx, args_a),
            self.vm_b.call(&mut ctx_b, func_idx, args_b),
        ))
        .map_err(SkipReason::from)?;

        let vec_a = match result_a {
            Some(val) => vec![val],
            None => vec![],
        };
        let vec_b = match result_b {
            Some(val) => vec![val],
            None => vec![],
        };
...
fn run_spec_test(wast_content: &str) -> TestStats {
    ...
    // Run private passes for each arg position (skip if PUBLIC_ONLY env var is set)
    if std::env::var("PUBLIC_ONLY").is_err() {
...
fn run_assert_return(
    ...
    // Build args for each VM. When a param is marked private, vm_a
    // holds the value as Private and vm_b holds it as Blind (its view
    // of the peer's private value). Flush will exchange it.
    ...
    let (result_a, result_b) = pair.execute(func_idx, args_a, args_b)?;
...
fn run_assert_trap(
    ...
    match pair.execute(func_idx, args_a, args_b) {
...
impl From<VmError> for SkipReason {
    fn from(err: VmError) -> Self {
        ...
        if err.is_unsupported() || err.is_symbolic_address() || err.is_symbolic_value() {
