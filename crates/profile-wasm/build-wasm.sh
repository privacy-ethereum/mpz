#!/bin/bash
# Build the in-browser profiler: compile profile-wasm to wasm32-unknown-unknown
...
CRATE_VER="$(cargo tree -p profile-wasm -i wasm-bindgen --target wasm32-unknown-unknown 2>/dev/null \
...
echo "Building profile-wasm for wasm32-unknown-unknown..."
cargo build -p profile-wasm --target wasm32-unknown-unknown --release

echo "Generating JS bindings (--target web)..."
wasm-bindgen --target web --no-typescript \
    --out-dir "$SCRIPT_DIR/pkg" \
    "$ROOT/target/wasm32-unknown-unknown/release/profile_wasm.wasm"
