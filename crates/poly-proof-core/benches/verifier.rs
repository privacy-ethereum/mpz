...
struct TestData {
    ...
    constraints: VerifierConstraints<Gf2_128>,
    /// Pre-generated verifier evaluations: (id, keys).
    evals: Vec<(ConstraintId, Vec<Gf2_128>)>,
    /// MAC key Δ.
    delta: Gf2_128,
    /// Prover's proof message (shape-correct; see `bench_verifier`).
    proof: ProofMessage<Gf2_128>,
    /// Verifier's VOPE share.
    vope: VerifierVope<Gf2_128>,
...
fn setup(num_evals: usize) -> TestData {
    ...
    let d = Verifier::new(delta, &constraints).unwrap().required_vopes();
    let proof = ProofMessage {
        coefficients: (0..d).map(|_| random_gf128(&mut rng)).collect(),
    };
    let vope = VerifierVope {
        sum: random_gf128(&mut rng),
    };
...
fn bench_verifier(c: &mut Criterion) {
    ...
    let num_evals = 50_000 * 232;
    {
        ...
        let batch: Vec<(ConstraintId, &[Gf2_128])> = data
            .evals
            .iter()
            .map(|(id, k)| (*id, k.as_slice()))
            .collect();

        let verifier = Verifier::new(data.delta, &data.constraints).unwrap();
