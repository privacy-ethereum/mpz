...
impl<'a> Verifier<'a, Committed> {
    ...
    pub fn new(delta: Gf2_64, keys: &'a [Gf2_64], adjust: &'a [bool]) -> Result<Self> {
    ...
    pub fn accumulate<R: RngCore>(self, rng: R) -> Verifier<'a, Accumulate<R>> {
        Verifier {
            ...
            state: Accumulate {
                ...
                rng,
                xy: Gf2_64Accumulator::default(),
                z: Gf2_64Accumulator::default(),
            },
        }
    }
}
...
impl<'a, R> Verifier<'a, Accumulate<R>> {
    ...
    pub fn finish(self) -> Result<(Gf2_64, [u8; 32])> {
        if self.cursor != self.adjust.len() {
            return Err(Error::tape_unconsumed(self.cursor, self.adjust.len()));
        }
        let w = self.state.xy.finish() + self.delta * self.state.z.finish();
...
impl<R: RngCore> Context for Verifier<'_, Accumulate<R>> {
    ...
    fn mul(&mut self, a: Gf2_64, b: Gf2_64) -> Gf2_64 {
        ...
        self.state.xy.fma(a * b, chi);
        self.state.z.fma(key, chi);
